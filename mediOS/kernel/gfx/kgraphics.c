...
void gfx_drawLine(unsigned int color, int x1, int y1, int x2, int y2,struct graphicsBuffer * buff)
{
    int numpixels;
    int i;
    int deltax, deltay;
        ...
        //printk("processing GFX fct: %x\n",cmd);
            switch(cmd) {
                ...
                case 0x102:                      /* void hidePlane(int vplane) */
                    ...
                    if(!debug_scr_state)
                        osd_setComponentConfig(buffers_comp[*int_data],0);
                    break;
                case 0x103:                      /* void showPlane(int vplane) */
                    buffers[*int_data]->enable=1;
                    if(!debug_scr_state)
                        osd_setComponentConfig(buffers_comp[*int_data],buffers[*int_data]->state|OSD_COMPONENT_ENABLE);
                    break;
                ...
                case 0x107:                      /* void setSize(int vplane,int width,int height,int bitsPerPixel) */
                    ...
                    if(!debug_scr_state)
                    {
                        osd_setComponentSize(buffers_comp[p_data->vplane], 2*buffers[p_data->vplane]->real_width, g_data->h);
                        osd_setComponentSourceWidth(buffers_comp[p_data->vplane], ((buffers[p_data->vplane]->width*p_data->bpp)/32)/8);
                    }
                    break;
                ...
                case 0x109:                      /* void setPos(int vplane,int x,int y) */
                    ...
                    if(!debug_scr_state)
                        osd_setComponentPosition(buffers_comp[p_data->vplane],g_data->x,g_data->y);
                    break;
                ...
                case 0x10C:
                    ...
                    osd_setComponentOffset(buffers_comp[*int_data],buffers[*int_data]->offset);
                    break;
                ...
                case 0x20D:
                    ...
                    osd_setPaletteRGB(g_data->x,g_data->y,g_data->w,g_data->h);
                    break;
