...
#define putPix(color,offset,buff) { \
    unsigned int __real_color = color << 24 | color << 16 | color << 8 | color; \
    outl(__real_color,offset);             \
    offset+=buff->width;            \
    outl(__real_color,offset);             \
}
...
void graphics8_clearScreen(unsigned int color, struct graphicsBuffer * buff)
{
    graphics8_FillRect(color,0,0,buff->width/4,buff->height/2,buff);
}

void graphics8_DrawPixel(unsigned int color, int x, int y, struct graphicsBuffer * buff)
{
    unsigned char * offset = getOffset_big(x,y,buff,unsigned char);
    putPix(color,offset,buff);
}

unsigned int graphics8_ReadPixel(int x, int y, struct graphicsBuffer * buff)
{
    return inb(getOffset_big(x,y,buff,unsigned char));
}
...
void graphics8_DrawRect(unsigned int color, int x, int y, int width, int height, struct graphicsBuffer * buff)
{
    ...
    graphics8_DrawHorizLine(color, width,offset);            
    offset+=buff->width;
    offset2=offset+width*4-4;
    for(i=1;i<height-1;i++)
    {
        putPix(color,offset,buff);
        putPix(color,offset2,buff);      
        offset+=buff->width;
        offset2+=buff->width;
    }
...
void graphics8_ScrollWindowVert(unsigned int bgColor, int x, int y, int width, int height, int scroll, int UP, struct graphicsBuffer * buff)
{
    ...
    src=getOffset_big(x,y,buff,unsigned char);

    if(UP)
    {
        inc=1;
        dest=src;
        src=src+(scroll*2)*buff->width;
    }
    else
    {
        inc=-1;
        dest=src+(height*2)*buff->width;
        src=dest-(scroll*2)*buff->width;
    }
    
    for(j=0;j<(height-scroll)*2;j++)
    {
...
void graphics8_ScrollWindowHoriz(unsigned int bgColor, int x, int y, int width, int height, int scroll, int RIGHT, struct graphicsBuffer * buff)
{
    ...
    if(bgColor!=-1)
    {
        if(RIGHT)
        {
            ...
            {
                memcpy(tmp,src,(width-scroll)*4);
                memcpy(dest,tmp,(width-scroll)*4);
        ...
        else
        {
            ...
            src=getOffset_big(x,y,buff,unsigned char);
            dest=src;
            offset=src+(width-scroll)*4;
            src=src+scroll*4;
            for(j=0;j<height*2;j++)
            {
                memcpy(dest,src,(width-scroll)*4);
                graphics8_DrawHorizLine(bgColor,scroll,offset);
                dest+=buff->width;
                src+=buff->width;
                offset+=buff->width;
...
void graphics8_DrawString(struct graphicsFont * font, unsigned int color,unsigned int bg_color, int x, int y, unsigned char * s, struct graphicsBuffer * buff)
{
    ...
    int i,j,k,shift,index;
    unsigned char * src;
    unsigned char * org=getOffset_big(x,y,buff,unsigned char);
    unsigned char * tmp;
    unsigned char * dest;
    unsigned int palette[2]={bg_color,color};
    while(*s)
