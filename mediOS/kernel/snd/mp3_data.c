...
#ifdef DEBUG_VERBOSE
    printk( "Header: %08x, Ver %d, lay %d, bitr %d, freq %d, "
...
static unsigned long __find_next_frame(int fd, int *offset, int max_offset, unsigned long last_header, int(*getfunc)(int fd, unsigned char *c))
{
    ...
    *offset = pos - 4;
    ...
    if(*offset)
        printk("Warning: skipping %d bytes of garbage\n", *offset);
#endif
...
int get_mp3file_info(int fd, struct mp3info *info)
{
    ...
    if (!memcmp(vbrheader, "VBRI", 4))
    {
        ...
        printk("%04x: %04x %04x ", 0, header >> 16, header & 0xffff);
        for(i = 4;i < (int)sizeof(frame)-4;i+=2) {
            if(i % 16 == 0) {
                printk("\n%04x: ", i-4);
            }
            printk("%04x ", (frame[i-4] << 8) | frame[i-4+1]);
        }
        ...
        printk("Frame size (%dkpbs): %d bytes (0x%x)\n",
               info->bitrate, info->frame_size, info->frame_size);
        printk("Frame count: %x\n", info->frame_count);
        printk("Byte count: %x\n", info->byte_count);
        printk("Offsets: %d\n", num_offsets);
        printk("Frames/entry: %d\n", frames_per_entry);
                       ...
                       void (*progressfunc)(int), int generate_toc)
{
    ...
    if(generate_toc)
    {
        ...
        for(i = 0;i < 100;i++) {
            ...
            printk("Pos %d: %d  relpos: %d  filepos: %x tocentry: %x\n",
