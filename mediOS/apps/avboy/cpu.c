...
__IRAM_CODE void div_advance(int cnt)
{
	cpu.div += (cnt<<1);
	if (cpu.div >= 256)
	{
...
__IRAM_CODE void timer_advance(int cnt)
{
	int unit, tima;
...
__IRAM_CODE void lcdc_advance(int cnt)
{
  cpu.lcdc -= cnt;
...
__IRAM_DATA union reg acc;
__IRAM_DATA byte b;
__IRAM_DATA word w;

__IRAM_CODE int cpu_emulate(int cycles)
{
	int i;
	byte op, cbop;
	int clen;
...
__IRAM_CODE int cpu_step(int max)
{
	int cnt;
