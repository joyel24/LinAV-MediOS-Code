...
void mem_updatemap()
{
	...
	{
		map[0x8] = lcd_vbank[R_VBK & 1] - 0x8000;
		map[0x9] = lcd_vbank[R_VBK & 1] - 0x8000;
	}
...
__IRAM_CODE byte ioreg_read(byte r)
{
	switch(r)
	{
	case RI_SC:
...
__IRAM_CODE byte mem_read(int a)
{
	int n;
	byte ha = (a>>12) & 0xE;
	...
	case 0x8:
		...
		return lcd_vbank[R_VBK&1][a & 0x1FFF];
	case 0xA:
