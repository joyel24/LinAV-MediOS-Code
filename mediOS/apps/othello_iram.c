...
extern struct position cursor_pos;
extern int stop_othello;
void iniBoard(void);
void endGame(void);
void iniCursorPos();
void redraw(void);

__IRAM_DATA int board[NB_CELL][NB_CELL];
__IRAM_DATA int allowedHuman[NB_CELL][NB_CELL];
__IRAM_DATA int nbPieces[2]={2,2};
__IRAM_DATA int endOfGame=0;
__IRAM_DATA int cursorMoveMode=0; /* 0 = move anywhere, 1 = move to possible locations */

__IRAM_DATA unsigned char human_bmap[12][12]= {
...
__IRAM_DATA unsigned char av3xx_bmap[12][12]= {
...
__IRAM_DATA BITMAP hBitmap = {(unsigned int) human_bmap, 12, 12, 0, 0};
__IRAM_DATA BITMAP aBitmap = {(unsigned int) av3xx_bmap, 12, 12, 0, 0};

#define PIECE_OFFSET (CELL_SIZE-PIECE_SIZE)/2+1


__IRAM_CODE void drawPiece(int x, int y, int player)
{
    switch(player)
    {
        case HUMAN:
...
__IRAM_CODE void erasePiece(int x, int y)
{
...
__IRAM_CODE int tstMove(int player,int x, int y,int dx, int dy,int dispMove)
{
...
__IRAM_CODE int validMove(int player,int x,int y,int chkAll,int dispMove)
{
    int other=(player==HUMAN?AV3XX:HUMAN);
...
__IRAM_CODE int computeAllowed(int allowed[NB_CELL][NB_CELL],int player)
{
    ...
    printf("out computeAllowed\n");
    return tot;
}

__IRAM_CODE int computerMove(void)
{
...
__IRAM_CODE void doMove(int x,int y)
{
    int tot;
...
__IRAM_CODE void simpleMove(int dx,int dy)
{
    int cx = cursor_pos.x;
    int cy = cursor_pos.y;
...
__IRAM_CODE void eventHandler(int evt)
{  
    if(evt != 0)
    {
