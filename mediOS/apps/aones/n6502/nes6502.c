...
extern char chaineDEB[256];

extern int nflip;					// Flip Index
extern uint32 stepbystep;
...
uint8 Rd6502_(uint32 Addr)
{	
	return Rd6502(Addr);
}

void Wr6502_(uint32 Addr,uint8 Value)
{
	sprintf(chaineDEB,"addr=%04X val=%02X old=%02X on sc=%d\nA=%02X X=%02X Y=%02X S=%02X",
		Addr,Value,Vnes.CPUMemory[Addr&0x7ff],ppu_currentscanline(),
		A,X,Y,S);
	Wr6502(Addr,Value);
}
...
#define PC_TO_PTR() \
	{ \
		uint32 t = (uint32)PC >> NES6502_BANKSHIFT; \
		/* last_bank_ptr = cpu.mem_page[t] - (t << NES6502_BANKSHIFT); */\
		last_bank_ptr = Vnes.CPUPageIndex[t] - (t << NES6502_BANKSHIFT); \
...
** The rest of the flags are stored as true bools.
*/
...
#define bank_writebyte(address, value) \
	Wr6502_(address, value)	

#define bank_readword(address) \
	(Rd6502_(address) | Rd6502_(address+1)<<8)	
	
#define slow_bank_readword(address) \
	(Rd6502_(address) | Rd6502_(address+1)<<8)	
#else
#define bank_readbyte(address) \
	Vnes.CPUPageIndex[address >> NES6502_BANKSHIFT][address & NES6502_BANKMASK]
	
#define slow_bank_readbyte(address) \
	Vnes.CPUPageIndex[address >> NES6502_BANKSHIFT][address & NES6502_BANKMASK] 	

#define bank_writebyte(address, value) \
	Vnes.CPUPageIndex[address >> NES6502_BANKSHIFT][address & NES6502_BANKMASK] = (value)

#define bank_readword(address) \
	(Vnes.CPUPageIndex[address >> NES6502_BANKSHIFT][address & NES6502_BANKMASK] | (Vnes.CPUPageIndex[(address+1) >> NES6502_BANKSHIFT][(address+1) & NES6502_BANKMASK]<<8))
...
void nes6502_setcontext(nes6502_context *context)
{
   ...
   ram = Vnes.CPUPageIndex[0];  /* quick zero-page/RAM references */
...
void nes6502_update_fast_pc()
{
	if (current_PC) {
		...
		*current_last_bank_ptr = Vnes.CPUPageIndex[t] - (t << NES6502_BANKSHIFT);
		*current_PC += (uint32)(*current_last_bank_ptr);
...
/*   GpRectFill(NULL,&gpDraw[0],0,120,320,100,0);     
  ...
  PC_TO_PTR();
	...
	*((uint8 *)PC-3),*((uint8 *)PC-2),*((uint8 *)PC-1),*((uint8 *)PC),
	*((uint8 *)PC+1),*((uint8 *)PC+2),*((uint8 *)PC+3),*((uint8 *)PC+4)); 
	if (stepbystep) 
	{
		debug_info();
		gpwait(0);
