...
//#include "tapwave.h"
#include "SoundMgr.h"
extern SndStreamRef snd_stream;
extern   reg_sys_t reg_sys;
Err lj_sndCallBack (void *userData,SndStreamRef stream, void *buffer,UInt32 frameCount);
#endif
   ...
   if (chan->dma_length)
   {
      ...
      while (chan->phaseacc < 0)
      {
         ...
         if (0 == (chan->dma_length & 7))
         {
            ...
            chan->cur_byte = Rd6502(chan->address);
            //chan->cur_byte = Vnes.CPUPageIndex[(chan->address)>>13][(chan->address)&0x1fff];
...
uint8 ex_read(uint32 address)
{
   ...
   else if(apu->ex_chip & 16)
   {
      ...
      #else
      d.timestamp = cpu_getcycles(FALSE);
      #endif
      ...
      while (num_samples--)
      {
         ...
         while((FALSE == EX_QEMPTY()) && apu->ex_queue[apu->ex_q_tail].timestamp <= elapsed_cycles)
         {
            ...
            else if(apu->ex_chip & 16)
            {
               ...
               {
                  /*gli uint8 dummy =*/ N106SoundRead(d->address & 0xFFFF);
               }
               else
               {
...
/* Overflow fixed by T.Yano Dec.12.2000 */
...
#ifdef __GP32__            
            *((uint8 *) buffer)++ = accum&0xFF;
            *((uint8 *) buffer)++ = (accum>>8)^128;
#else
   ...
   /* resync cycle counter */
      ...
      #else
      apu->elapsed_cycles = cpu_getcycles(FALSE);
      #endif   
}
...
int Open_SoundSys(int freq,int filter)
{  
  switch (freq)
  {
  	case 1:
		Vnes.var.snd_frequency=22050;
		break;
	case 2:
		Vnes.var.snd_frequency=11025;
	default:
		Vnes.var.snd_frequency=44100;
		break;		
  }

  Vnes.var.SoundStereo=0;
  Vnes.var.Sound16bits=1;

  apu = apu_create(Vnes.var.snd_frequency, Vnes.var.fps, 0, 8<<Vnes.var.Sound16bits);
  if(apu)
  {
...
void SNSSSaveStateSound(SnssSoundBlock *soundBlock)
{
  if(apu)
  {
    // copy in the per-channel stored values
    soundBlock->soundRegisters[APU_WRA0-0x4000] = apu->apus.rectangle[0].regs[0];
    soundBlock->soundRegisters[APU_WRA1-0x4000] = apu->apus.rectangle[0].regs[1];
    soundBlock->soundRegisters[APU_WRA2-0x4000] = apu->apus.rectangle[0].regs[2];
    soundBlock->soundRegisters[APU_WRA3-0x4000] = apu->apus.rectangle[0].regs[3];
    soundBlock->soundRegisters[APU_WRB0-0x4000] = apu->apus.rectangle[1].regs[0];
    soundBlock->soundRegisters[APU_WRB1-0x4000] = apu->apus.rectangle[1].regs[1];
    soundBlock->soundRegisters[APU_WRB2-0x4000] = apu->apus.rectangle[1].regs[2];
    soundBlock->soundRegisters[APU_WRB3-0x4000] = apu->apus.rectangle[1].regs[3];
    soundBlock->soundRegisters[APU_WRC0-0x4000] = apu->apus.triangle.regs[0];
    soundBlock->soundRegisters[APU_WRC2-0x4000] = apu->apus.triangle.regs[1];
    soundBlock->soundRegisters[APU_WRC3-0x4000] = apu->apus.triangle.regs[2];
    soundBlock->soundRegisters[APU_WRD0-0x4000] = apu->apus.noise.regs[0];
    soundBlock->soundRegisters[APU_WRD2-0x4000] = apu->apus.noise.regs[1];
    soundBlock->soundRegisters[APU_WRD3-0x4000] = apu->apus.noise.regs[2];
    soundBlock->soundRegisters[APU_WRE0-0x4000] = apu->apus.dmc.regs[0];
    soundBlock->soundRegisters[APU_WRE1-0x4000] = apu->apus.dmc.regs[1];
    soundBlock->soundRegisters[APU_WRE2-0x4000] = apu->apus.dmc.regs[2];
    soundBlock->soundRegisters[APU_WRE3-0x4000] = apu->apus.dmc.regs[3];
  }
}

void SNSSLoadStateSound(SnssSoundBlock *soundBlock)
{
}
