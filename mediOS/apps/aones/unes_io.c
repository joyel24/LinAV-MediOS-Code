...
#ifdef __GP32__
extern GPDRAWSURFACE gpDraw[2];		// Priamry & Back SURFACE
...
static uint8 pad_strobe;
static uint32 pad1_bits,pad2_bits;
...
{	
  ...
  Addr&=0xffff;
  //sprintf(strtmp,"Write %02X at : %04X",Value,Addr);
//  DrawMessage(strtmp,1);
...
//  DEBUGS("read");
  ...
  Addr&=0xffff;
  //sprintf(strtmp,"Read at : %04X",Addr);
  //DrawMessage(strtmp,1);
