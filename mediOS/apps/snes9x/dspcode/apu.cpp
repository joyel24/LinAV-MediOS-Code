...
bool8 S9xInitAPU ()
{
    IAPU.Slowdown = 0; /* VP : reset slowdown */
...
//     ZeroMemory (IAPU.CachedSamples, 0x40000);
    ...
    int i;
    for (i=0; i<sizeof(APUROM); i++)
      SET(i, APUROM[i]);
    memmove (APU.ExtraRAM, APUROM, sizeof (APUROM));
    IAPU.PC = /*IAPU.RAM +*/ GET(0xfffe) + (GET (0xffff) << 8);
    APU.Cycles = 0;
    SETW(0);
    APURegisters.X = 0;
    APURegisters.S = 0xff;
...
//     if (reg > 0x80) {
//       // VP safety net, commands above 0x80 are used to control other parts (frame synchronisation, tracing, debugging, etc ...)
//       return;
//     }
//     queue_dsp((reg<<8) | byte);
	...
	if (byte != APU.DSP [APU_MVOL_RIGHT])
	{
		...
		S9xTraceSoundDSP ("[%d] Master volume right:%d\n",
				  ICPU.Scanline, (signed char) byte);
#endif
		S9xSetMasterVolume (APU.DSP [APU_MVOL_LEFT],
				    byte);
	}
	break;
    case APU_EVOL_LEFT:
	if (byte != APU.DSP [APU_EVOL_LEFT])
	{
		...
		S9xTraceSoundDSP ("[%d] Echo volume left:%d\n",
				  ICPU.Scanline, (signed char) byte);
#endif
		S9xSetEchoVolume (byte,
				  APU.DSP [APU_EVOL_RIGHT]);
	}
	break;
    case APU_EVOL_RIGHT:
	if (byte != APU.DSP [APU_EVOL_RIGHT])
	{
		...
		S9xTraceSoundDSP ("[%d] Echo volume right:%d\n",
				  ICPU.Scanline, (signed char) byte);
#endif
		S9xSetEchoVolume (APU.DSP [APU_EVOL_LEFT],
				  byte);
	}
	break;
    case APU_ENDX:
	...
	if (byte != APU.DSP [reg])
	{
		...
		S9xTraceSoundDSP ("[%d] %d volume right: %d\n", 
				  ICPU.Scanline, reg >>4, (signed char) byte);
#endif
		S9xSetSoundVolume (reg >> 4, APU.DSP [reg - 1],
				   byte);
	}
	break;
...
void S9xSetAPUControl (uint8 byte)
{
  // VP FIXME
	...
	APU.Timer [0] = 0;
	SET(0xfd, 0);
	if ((APU.TimerTarget [0] = GET(0xfa)) == 0)
	...
	APU.Timer [1] = 0;
	SET(0xfe, 0);
	if ((APU.TimerTarget [1] = GET(0xfb)) == 0)
	...
	APU.Timer [2] = 0;
	SET(0xff, 0);
	if ((APU.TimerTarget [2] = GET(0xfc)) == 0)
    ...
    if (byte & 0x10) {
      SET(0xF4, 0); SET(0xF5, 0);
    }

    if (byte & 0x20) {
      SET(0xF6, 0); SET(0xF7, 0);
    }
