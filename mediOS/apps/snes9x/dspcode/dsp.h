...
struct voice_state
{
  ...
  signed short smp1,smp2;/* last two values decoded (for filter) */
  int
    //mem_ptr,		/* where in memory is sample data to read */
  ...
  short sampbuf[16+3];	/* Buffer for Gaussian interpolation */
...
void DSP_Update(short *, int count);
void DSP_Reset(void);
void DSP_Init(void);
