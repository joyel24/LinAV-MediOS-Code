...
static void handle_type_other(char *filename)
{
	...
	if (ext == 0) {
		GrText(browser_wid, browser_gc, 8, yPosDebug,	"no extension!", -1, GR_TFASCII);
		yPosDebug+=10;
      new_exe_window(filename);
//		new_textview_window(filename);
	}
	else if (is_image_type(ext)) {
		GrText(browser_wid, browser_gc, 8, yPosDebug,	"image!", -1, GR_TFASCII);
		yPosDebug+=10;
		new_image_window(filename);
	}
	else if (is_text_type(ext)) {
		GrText(browser_wid, browser_gc, 8, yPosDebug,	"text!", -1, GR_TFASCII);
		yPosDebug+=10;
		new_textview_window(filename);
	}
	else if (is_mp3_type(ext)) {
		GrText(browser_wid, browser_gc, 8, yPosDebug,	"mp3!", -1, GR_TFASCII);
		yPosDebug+=10;
		new_mp3_window(filename);
	}
	else if (is_raw_audio_type(ext)) {
		GrText(browser_wid, browser_gc, 8, yPosDebug,	"raw!", -1, GR_TFASCII);
		yPosDebug+=10;
		new_playback_window(filename);
	}
	else if (is_script_type(ext)) {
		GrText(browser_wid, browser_gc, 8, yPosDebug,	"script!", -1, GR_TFASCII);
		yPosDebug+=10;
		new_script_window(filename);
	}
	else  {
		GrText(browser_wid, browser_gc, 8, yPosDebug,	"message!", -1, GR_TFASCII);
		yPosDebug+=10;
   ...
   pwd = get_current_dir_name();
	...
	if (pid == 0) { // child side
      ...
      strcat(pwd, filename);
		...
		err = execl(pwd, filename);
//		err = execl("/mnt/LinAv/snow", "snow");
		sprintf(tmp,"Execl-Error:  %d",err);
		GrText(browser_wid, browser_gc, 8, yPosDebug, tmp, strlen(tmp), GR_TFASCII);
		fprintf(stderr, "exec failed!%s %s %d %ld\n", pwd, filename, err, errno);
		exit(1);
	}
	else {
		if (pid > 0) {
