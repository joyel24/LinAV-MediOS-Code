...
*/
    ...
    printf("[init disk] %d disk mounted\n",nb_mounted);
}
...
int disk_mount(int drive)
{
    ...
    printf("No partition found, trying to mount sector 0.\n");
...
struct partition_info * setup_disk(int drive)
{
    ...
    if ( (sector[510] != 0x55) ||
         (sector[511] != 0xaa)) {
        printf("Bad boot sector signature\n");
        return NULL;
    }
    ...
    for ( i=0; i<4; i++ ) {
        ...
        if(j<9)
                ...
                printf("Error: partition type not supported: %x\n",pinfo[i].type);
               
        printf("Part%d: start=%08x, size=%08x, type:%s (%02x)\n",i,
...
void printPartition_info(struct partition_info * partition_list)
{
    int i;
    for(i=0;i<4;i++)
	printf("Partition %d: start=%x, size=%x, type:%s (%x)\n",i,
