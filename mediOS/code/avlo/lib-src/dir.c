...
DIR* opendir(const char* name)
{
    ...
    if ( name[0] != '/' ) {
        printf("Only absolute paths supported right now\n");
        return NULL;
    }
    ...
    if ( fat_opendir(volume, &pdir->fatdir, 0, NULL) < 0 ) {
        printf("Failed opening root dir\n");
        pdir->busy = false;
        return NULL;
    }
...
int mkdir(const char *name, int mode)
{
    ...
    if ( name[0] != '/' ) {
        printf("mkdir: Only absolute paths supported right now\n");
        return -1;
    }
    ...
    *end = 0;
    ...
    printf("mkdir: parent: %s, name: %s\n", parent, basename);

    dir = opendir(parent);

    if(!dir) {
        printf("mkdir: can't open parent dir\n");
        return -2;
    }

    if(basename[0] == 0) {
        printf("mkdir: Empty dir name\n");
        return -3;
    }
