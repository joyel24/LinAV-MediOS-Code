...
void HW_timer::write(uint32_t addr,uint32_t val,int size)
{
    switch(addr&0xF)
    {
        case 0x0:
            ...
            break;
        case 0x2:
            DEBUG_HW("%s%d write ClockSelect(%x)=%s\n",name,timer_num,val&0x1,(val&0x1)==0x1?"Ext or SYSCLK clock":"Current ARM clock");
            tm_clk=val&0x1;
            break;
        case 0x4:
            DEBUG_HW("%s%d write Prescaler divide value=%x\n",name,timer_num,val&0x3FF);
            tm_p_scaler=val&0x3FF;
            break;
        case 0x6:
            DEBUG_HW("%s%d write Max counter val=%x\n",name,timer_num,val&0xFFFF);
            tm_max=val&0xFFFF;
            break;
        case 0x8:
            DEBUG_HW("%s%d write Timer Triger => %s\n",name,timer_num,(val&0x1)==0x1?"GO":"NO GO");
            tm_trigger=val&0x1;
            break;
        case 0xA:
            DEBUG_HW("%s%d write Timer count => SHOULD NOT READ\n",name,timer_num);
            break;
        default:
            DEBUG_HW("%s%d UKN write @0x%08x, size %x\n",name,timer_num,addr,size);
            break;
    }
}
