/*
*   loader/target/arch_AV3XX/gui.c
*
*   AvLo - linav project
...
{
    int x,y,h,w;
    char * str = "USB MODE - Press F3 when done";
    printf("%s\n",str);
    setFont(BOX_FONT);
    getStringS(str,&w,&h);
    drawBox(w,h,&x,&y);
...
void printFault(int faultNum)
{
    ...
    printf("%s\n",errorMsg[faultNum]);
    putS(COLOR_TXT,COLOR_BOX,x,y+h,"USB is ON - HD can be accessed");
