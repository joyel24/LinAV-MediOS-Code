...
static int update_fsinfo(struct bpb* fat_bpb);
static int flush_fat(struct bpb* fat_bpb);
static int bpb_is_sane(struct bpb* fat_bpb);
static void *cache_fat_sector(struct bpb* fat_bpb, int secnum, bool dirty);
static int create_dos_name(const unsigned char *name, unsigned char *newname);
static unsigned int find_free_cluster(struct bpb* fat_bpb, unsigned int start);
static int transfer(struct bpb* fat_bpb, unsigned int start, int count, char* buf, bool write );
...
static int cluster2sec(struct bpb* fat_bpb, int cluster)
{
    ...
    {
        printf( "cluster2sec() - Bad cluster number (%d)\n", cluster);
        return -1;
    }
...
int fat_startsector(int volume)
{
    struct bpb* fat_bpb = &fat_bpbs[volume];
    return fat_bpb->startsector;
}

void fat_size(int volume, unsigned int* size, unsigned int* free)
{
    struct bpb* fat_bpb = &fat_bpbs[volume];
    if (size)
...
int fat_mount(int volume, int drive, int startsector)
{
    struct bpb* fat_bpb = &fat_bpbs[volume];
    ...
    if(rc)
    {
        printf( "fat_mount() - Couldn't read BPB (error code %d)\n", rc);
        return rc * 10 - 1;
    }
    ...
    if (rc < 0)
    {
        printf( "fat_mount() - BPB is not sane\n");
        return rc * 10 - 3;
    }
    ...
    else
    {
        ...
        if (rc < 0)
        {
            printf( "fat_mount() - Couldn't read FSInfo (error code %d)\n", rc);
            return rc * 10 - 4;
        }
    ...
    {
        fat_recalc_free(volume);
    }

    printf("Freecount: %d\n",fat_bpb->fsinfo.freecount);
    printf("Nextfree: 0x%x\n",fat_bpb->fsinfo.nextfree);
    printf("Cluster count: 0x%x\n",fat_bpb->dataclusters);
    printf("Sectors per cluster: %d\n",fat_bpb->bpb_secperclus);
    printf("FAT sectors: 0x%x\n",fat_bpb->fatsize);
    printf("FAT root dir sectors: 0x%x - root cluster: 0x%x\n",fat_bpb->rootdirsector, fat_bpb->bpb_rootclus);
...
void fat_recalc_free(int volume)
{
    struct bpb* fat_bpb = &fat_bpbs[volume];
    int free = 0;
    unsigned i;
    if (fat_bpb->is_fat16)
    {
        for (i = 0; i<fat_bpb->fatsize; i++) {
            unsigned int j;
            unsigned short* fat = cache_fat_sector(fat_bpb, i, false);
    ...
    else
    {
        for (i = 0; i<fat_bpb->fatsize; i++) {
            unsigned int j;
            unsigned int* fat = cache_fat_sector(fat_bpb, i, false);
...
static int bpb_is_sane(struct bpb* fat_bpb)
{
    ...
    {
        printf( "bpb_is_sane() - Error: sector size is not 512 (%d)\n",
                fat_bpb->bpb_bytspersec);
        return -1;
    }
    if(fat_bpb->bpb_secperclus * fat_bpb->bpb_bytspersec > 128*1024)
    {
        printf( "bpb_is_sane() - Error: cluster size is larger than 128K "
                "(%d * %d = %d)\n",
    ...
    {
        printf( "bpb_is_sane() - Warning: NumFATS is not 2 (%d)\n",
                fat_bpb->bpb_numfats);
    }
    if(fat_bpb->bpb_media != 0xf0 && fat_bpb->bpb_media < 0xf8)
    {
        printf( "bpb_is_sane() - Warning: Non-standard "
                "media type (0x%02x)\n",
                fat_bpb->bpb_media);
    }
    if(fat_bpb->last_word != 0xaa55)
    {
        printf( "bpb_is_sane() - Error: Last word is not "
                "0xaa55 (0x%04x)\n", fat_bpb->last_word);
        return -3;
    }
    ...
    if (fat_bpb->fsinfo.freecount >
        ...
        fat_bpb->bpb_secperclus)
    {
        printf( "bpb_is_sane() - Error: FSInfo.Freecount > disk size "
                 "(0x%04x)\n", fat_bpb->fsinfo.freecount);
        return -4;
    }
                             ...
                             unsigned char *sectorbuf)
{
    ...
    if(rc < 0)
    {
        printf("flush_fat_sector() - Could not write sector %d"
               " (error %d)\n",
               secnum, rc);
    }
...
static void *cache_fat_sector(struct bpb* fat_bpb,
                              int fatsector, bool dirty)
{
    ...
    if(!fce->inuse)
    {
        ...
        if(rc < 0)
        {
            printf( "cache_fat_sector() - Could not read sector %d"
                    " (error %d)\n", secnum, rc);
            //mutex_unlock(&cache_mutex);
            return NULL;
        }
...
static unsigned int find_free_cluster(struct bpb* fat_bpb, unsigned int startcluster)
{
    unsigned int sector;
    unsigned int offset;
    unsigned int i;
    ...
    if (fat_bpb->is_fat16)
    {
        ...
        for (i = 0; i<fat_bpb->fatsize; i++) {
            ...
            unsigned short* fat = cache_fat_sector(fat_bpb, nr, false);
            if ( !fat )
                break;
            for (j = 0; j < CLUSTERS_PER_FAT16_SECTOR; j++) {
    ...
    else
    {
        ...
        for (i = 0; i<fat_bpb->fatsize; i++) {
            unsigned int j;
            unsigned int nr = (i + sector) % fat_bpb->fatsize;
            unsigned int* fat = cache_fat_sector(fat_bpb, nr, false);
            if ( !fat )
                break;
            for (j = 0; j < CLUSTERS_PER_FAT_SECTOR; j++) {
    ...
    //printf("find_free_cluster(%x) == 0\n",startcluster);
    return 0; /* 0 is an illegal cluster number */
}

static int update_fat_entry(struct bpb* fat_bpb, unsigned int entry, unsigned int val)
{
    if (fat_bpb->is_fat16)
    {
        ...
        //printf("update_fat_entry(%x,%x)\n",entry,val);

        if (entry==val)
            printf("Creating FAT loop: %x,%x\n",entry,val);

        if ( entry < 2 )
            printf("Updating reserved FAT entry %d.\n",entry);

        sec = cache_fat_sector(fat_bpb, sector, true);
        if (!sec)
        {
            printf( "update_fat_entry() - Could not cache sector %d\n", sector);
            return -1;
        }
        ...
        //printf("update_fat_entry: %d free clusters\n", fat_bpb->fsinfo.freecount);
...
static int read_fat_entry(struct bpb* fat_bpb, unsigned int entry)
{
    if (fat_bpb->is_fat16)
    {
        int sector = entry / CLUSTERS_PER_FAT16_SECTOR;
        ...
        sec = cache_fat_sector(fat_bpb, sector, false);
        if (!sec)
        {
            printf( "read_fat_entry() - Could not cache sector %d\n", sector);
            return -1;
        }
...
static int get_next_cluster(struct bpb* fat_bpb, int cluster)
{
    int next_cluster;
    ...
    next_cluster = read_fat_entry(fat_bpb, cluster);
...
static int update_fsinfo(struct bpb* fat_bpb)
{
    ...
    if (rc < 0)
    {
        printf( "flush_fat() - Couldn't read FSInfo (error code %d)\n", rc);
        return rc * 10 - 1;
    }
    ...
    *intptr = SWAB32(fat_bpb->fsinfo.nextfree);
    ...
    if (rc < 0)
    {
        printf( "flush_fat() - Couldn't write FSInfo (error code %d)\n", rc);
        return rc * 10 - 2;
    }
                           ...
                           bool is_directory)
{
    ...
    /*printf("write_long_name(file:%x, first:%d, num:%d, name:%s)\n",
    ...
    for (i=0; i < numentries; i++) {
        ...
        if ( idx >= DIR_ENTRIES_PER_SECTOR ) {
            ...
            rc = fat_readwrite(file, 1, buf, false);
            if (rc<0) {
                //printf("Failed writing new sector: %d\n",rc);
                return rc * 10 - 5;
            }
            if (rc==0)
                /* end of dir */
        ...
        if (entry[0] && entry[0] != 0xe5 )
            printf("Dir entry %d in sector %x is not free! "
                   "%02x %02x %02x %02x",
                   idx, sector,
        ...
        if ( i+1 < numentries ) {
            ...
            //printf("Longname entry %d: %.13s\n", idx, name+nameidx);
        }
        else {
            /* shortname entry */
            unsigned short date=0, time=0, tenth=0;
            //printf("Shortname entry: %.13s\n", shortname);
 ...
 restart:
    ...
    for (sector=0; !done; sector++)
    {
        ...
        if (rc == 0) {
            ...
            memset(buf, 0, sizeof buf);
            //printf("Adding new sector to dir\n");
        ...
        if (rc < 0) {
            /*printf("add_dir_entry() - Couldn't read dir"
                    " (error code %d)\n", rc);*/
            return rc * 10 - 4;
        }
...
#warning rand needed when duplicated short name
                    printf("Duplicate shortname, changing to %s\n",
                            shortname);
    ...
    /*printf("Adding longname to entry %d in sector %d\n",
            firstentry, sector);*/
    ...
    /*printf("Added new dir entry %d, using %d slots.\n",
    ...
    if (last)
    {
        ...
        /*printf("Updating end-of-dir entry %d\n",lastentry);*/
...
static int update_short_entry( struct fat_file* file, int size, int attr )
{
    ...
    /*printf("update_file_size(cluster:%x entry:%d size:%d)\n",
            file->firstcluster, file->direntry, size);*/

    /* create a temporary file handle for the dir holding this file */
    rc = fat_open(file->volume, file->dircluster, &dir, NULL);
    if (rc < 0)
        return rc * 10 - 1;
    ...
    if (!entry[0] || entry[0] == 0xe5)
        printf("Updating size on empty dir entry %d\n", file->direntry);
             ...
             const struct fat_dir* dir)
{
    ...
    {
        printf("fat_open() illegal volume %d\n", volume);
        return -1;
    }
...
int fat_truncate(const struct fat_file *file)
{
    ...
    //printf("fat_truncate(%x, %x)\n", file->firstcluster, last);

    for ( last = get_next_cluster(fat_bpb, last); last; last = next ) {
        next = get_next_cluster(fat_bpb, last);
        update_fat_entry(fat_bpb, last,0);
    }
    if (file->lastcluster)
        update_fat_entry(fat_bpb, file->lastcluster,FAT_EOF_MARK);
...
int fat_closewrite(struct fat_file *file, int size, int attr)
{
    ...
    //printf("fat_closewrite(size=%d)\n",size);

    if (!size) {
        /* empty file */
        if ( file->firstcluster ) {
            update_fat_entry(fat_bpb, file->firstcluster, 0);
            file->firstcluster = 0;
        }
    }
...
static int free_direntries(struct fat_file* file)
{
    ...
    for (i=0; i < numentries; i++) {
        /*printf("Clearing dir entry %d (%d/%d)\n",
                ...
                int attr)
{
    ...
    if (file->volume != dir->file.volume) {
        printf("No rename across volumes!\n");
        return -1;
    }


    if ( !file->dircluster ) {
        printf("File has no dir cluster!\n");
        return -2;
    }
                              ...
                              int* newsector)
{
    ...
    if (!cluster) {
        ...
        if (cluster) {
            ...
            printf("next_write_cluster(): Disk full!\n");
            return 0;
        }
    }
    sector = cluster2sec(fat_bpb, cluster);
    if (sector<0)
        return 0;
...
static int transfer(struct bpb* fat_bpb,
                    unsigned int start, int count, char* buf, bool write )
{
    int rc;

    /*printf("transfer(s=%x, c=%x, %s)\n",
    ...
    if (write) {
        ...
        if (start < firstallowed)
            printf("Write %d before data\n", firstallowed - start);
        if (start + count > fat_bpb->totalsectors)
            printf("Write %d after data\n",
    ...
    if (rc < 0) {
        printf( "transfer() - Couldn't %s sector %x"
                " (error code %d)\n",
                   ...
                   void* buf, bool write )
{
    ...
    /*printf( "fat_readwrite(file:%x,count:0x%x,buf:%x,%s)\n",
             file->firstcluster,sectorcount,buf,write?"write":"read");*/
    /*printf( "fat_readwrite: sec=%x numsec=%d eof=%d\n",
    ...
    for (i=0; (i < sectorcount) && (sector > -1); i++ ) {
        ...
        if ( ((sector != first) && (sector != last+1)) || /* not sequential */
            ...
            rc = transfer(fat_bpb, first, count, buf, write );
            if (rc < 0)
                return rc * 10 - 1;
...
int fat_seek(struct fat_file *file, unsigned int seeksector )
{
    ...
    if (seeksector) {
        ...
        for (i=0; i<numclusters; i++) {
            cluster = get_next_cluster(fat_bpb, cluster);
            if (!cluster) {
                printf("Seeking beyond the end of the file! "
                       "(sector %d, cluster %d)\n", seeksector, i);
                return -1;
            }
        }
                ...
                const struct fat_dir *parent_dir)
{
    ...
    rc = fat_open(volume, startcluster, &dir->file, parent_dir);
    if(rc)
    {
        printf( "fat_opendir() - Couldn't open dir"
                " (error code %d)\n", rc);
        return rc * 10 - 1;
    }
...
int fat_getnext(struct fat_dir *dir, struct fat_direntry *entry)
{
    ...
    while(!done)
    {
        if ( !(dir->entry % DIR_ENTRIES_PER_SECTOR) || !dir->sector )
        {
            ...
            if (rc < 0) {
                printf( "fat_getnext() - Couldn't read dir"
                        " (error code %d)\n", rc);
                return rc * 10 - 1;
            }
...
int fat_get_cluster_size(int volume)
{
    struct bpb* fat_bpb = &fat_bpbs[volume];
