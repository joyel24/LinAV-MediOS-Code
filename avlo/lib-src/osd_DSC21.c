...
void osdRestorePlane(int component, unsigned int address, int x, int y, int w, int h, int bpp, int state,int enable)
{
    ...
    osdSetComponentSize (component, 2*w, h);
    osdSetComponentPosition(component,x,y);
    osdSetComponentSourceWidth(component,(((w*bpp)/32)/8));
    
    if(enable)
    {
        osdSetComponentConfig(component,state|OSD_COMPONENT_ENABLE(component));
    }
}
