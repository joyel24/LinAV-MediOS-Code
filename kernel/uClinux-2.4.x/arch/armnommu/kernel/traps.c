...
static void dump_tsk_stack(struct task_struct *tsk, unsigned long sp)
{
	printk("Stack:\n");
...
NORET_TYPE void die(const char *str, struct pt_regs *regs, int err)
{
	...
	if (!user_mode(regs) || in_interrupt()) {
		...
		dump_tsk_stack(tsk, (unsigned long)(regs + 1));
...
{
#if defined(CONFIG_ARCH_POLDHU) || defined(CONFIG_ARCH_ISL3893)
#ifdef ISL3893_NO_MVC
	extern void __trap_init(void *);

	printk("3893: Initializing Vectors at address 0...");
	/* Put the vectors at address 0 */
	__trap_init((void *)0);
	printk("Done\n");
#else /* ISL3893_NO_MVC */
	/*
	 * this is put into the MAC Virtual Coprocessor
	 */
	extern void(*vector_undefinstr)(void);
	extern void(*vector_swi)(void);
	extern void(*vector_prefetch)(void);
	extern void(*vector_data)(void);
	extern void(*vector_IRQ)(void);

	int ret;
	static struct msg_init blob_init_msg;

	blob_init_msg.und_handler = (void(*)(void))&vector_undefinstr;
	blob_init_msg.swi_handler = (void(*)(void))&vector_swi;
	blob_init_msg.prf_handler = (void(*)(void))&vector_prefetch;
	blob_init_msg.dat_handler = (void(*)(void))&vector_data;
	blob_init_msg.irq_handler = (void(*)(void))&vector_IRQ;

	ret = blob_init(&blob_init_msg);
	if(ret < 0) {
		printk(KERN_CRIT "Could not initialize MVC (code %d)\n", ret);
		panic("MVC init failed");
	}
