...
/*
  ...
  fix_window()    applies a Hanning window to the (time) input
  ...
  fix_iscale()    scale an integer value by (numer/denom).
...
static fixed Sinewave[]; /* placed at end of this file for clarity */
static int fft(fixed fr[], fixed fi[], int m);
...
/*   return a; */
/* } */
#define fix_mpy(A, B) (((int)(A) * (int)(B))>>15)
...
{
  return inverse ? ifft(fr,fi,m) : fft(fr,fi,m);
}
...
__IRAM_CODE void fix_window(fixed fr[], int n)
{
  int i, j, k;
