...
#define TCX_RAM24BIT            0x01000000
        switch (fb_fix.accel) {
            case FB_ACCEL_SUN_CGTHREE:
	         psd->addr = mmap(NULL, psd->size, PROT_READ|PROT_WRITE,0,fb,CG3_MMAP_OFFSET); /* changed arg 4 from MAP_SHARED to 0 (in 4 mmap)*/
                 break;
            case FB_ACCEL_SUN_CGSIX:
	         psd->addr = mmap(NULL, psd->size, PROT_READ|PROT_WRITE,0,fb,CG6_RAM);
                 break;
	    case FB_ACCEL_SUN_TCX:
	         psd->addr = mmap(NULL, psd->size, PROT_READ|PROT_WRITE,0,fb,TCX_RAM24BIT);
                 break;
            default:
