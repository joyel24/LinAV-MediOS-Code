...
__IRAM_CODE void DecodeBlock (Channel *ch)
{
    int32 out;
...
static __IRAM_DATA int wave[SOUND_BUFFER_SIZE];

__IRAM_CODE void MixStereo (int sample_count)
{
	...
	VL = (ch->sample * ch-> left_vol_level) / (128*128);
	VR = (ch->sample * ch->right_vol_level) / (128*128);
	...
	for (uint32 I = 0; I < (uint32) sample_count; I += 2)
	{
		...
		ch-> left_vol_level = (ch->envx * ch->volume_left)/* / (128*128)*/;
		ch->right_vol_level = (ch->envx * ch->volume_right)/* / (128*128)*/;
		VL = (ch->sample * ch-> left_vol_level) / (128*128);
		VR = (ch->sample * ch->right_vol_level) / (128*128);
...
#if 0
__IRAM_CODE void MixMono (int sample_count)
{
    int pitch_mod = SoundData.pitch_mod & (~APU.DSP[APU_NON]);
...
__IRAM_CODE void S9xMixSamplesO (uint8 *buffer, int sample_count, int byte_offset)
{
    int J;
    int I;
