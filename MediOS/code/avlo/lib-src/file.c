...
int fopen(const char* pathname, int flags)
{
    ...
    if ( pathname[0] != '/' ) {
        ...
        printf("Only absolute pathnames supported at the moment\n");
        return -1;
    }
    ...
    if ( !entry ) {
        printf("Didn't find file %s (read %d entry)\n",name,rc);
        if ( file->write && (flags & O_CREAT) ) {
            rc = fat_create_file(name,
                                 &(file->fatfile),
                                 &(dir->fatdir));
            if (rc < 0) {
                printf("Couldn't create %s in %s\n",name,pathnamecopy);
                file->busy = false;
                closedir(dir);
                return rc * 10 - 6;
            }
...
static int flush_cache(int fd)
{
    ...
    //printf("Flushing dirty sector cache\n");
...
static int readwrite(int fd, void* buf, int count, bool write)
{
    ...
    if ( sectors ) {
        ...
        if ( rc < 0 ) {
            printf("Failed read/writing %d sectors\n",sectors);
