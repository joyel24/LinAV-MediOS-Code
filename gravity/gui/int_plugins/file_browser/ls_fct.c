/*
* kernel/gui/internal_plugin/ls_fct.c
*
*   AMOS project
...
{
    int i;
    printf("[cleanList] %d entries to be cleaned (%08x)\n",bdata->listused,bdata->list);
    if(bdata->list)
    {
...
int doLs(struct browser_data * bdata)
{
    ...
    if(!ini_lists(bdata))
        ...
        printf( "[dols] opening %s\n",bdata->path);
        
    dirp = opendir(bdata->path);   
    
    if(!dirp)
    {
        printf( "[dols] error opening dir\n");
        return 0;
    }
    ...
    printf( "[dols] folder parsed: find %d files %d folder => %d entires\n",bdata->nbFile,bdata->nbDir,bdata->listused);
...
int upDir(struct browser_data * bdata)
{
    ...
    if(isRoot(bdata))
    {
        printf("[upDir] cant go up, already at root (%s)\n",bdata->path);
        return 0;
    }
    else
    {
        ...
        if(!ptr)
        {
            printf("[upDir] error can't find a '/' (%s)\n",bdata->path);
            return 0;
        }
...
int inDir(struct browser_data * bdata,char * name)
{
    ...
    if(len+strlen(name)+2<PATHLEN)
    {
        if(isRoot(bdata))
        {
            printf("[inDir] from root\n");
            strcpy(bdata->path+len,name);
        }
        else
        {
           printf("[inDir] not from root\n");
           bdata->path[len]='/';
    ...
    {
        printf("[inDir] error cant go to %s from %s, string too long\n",name,bdata->path);
        return 0;
    }
}
