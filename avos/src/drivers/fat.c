...
static int sec2cluster(unsigned int sec)
{
    ...
    {
        uartOuts("sec2cluster() - Bad sector number\n");
...
int fat_mount(int startsector)
{
    ...
    /* Read the sector */
    rc = ataReadSectors(startsector,1,buf);
    if(rc)
    {
        uartOuts("fat_mount() - Couldn't read BPB\n");
...
//TODO
    ...
    */
...
#endif
    {
        uartOuts("This is not FAT32. Go away!");
    ...
    if (rc < 0)
    {
        uartOuts("fat_mount() - BPB is not sane");
    ...
    rc = ataReadSectors(startsector + fat_bpb.bpb_fsinfo, 1, buf);
    if (rc < 0)
    {
        uartOuts("fat_mount() - Couldn't read FSInfo");
...
static int bpb_is_sane(void)
{
    ...
    {
        uartOuts("bpb_is_sane() - Error: sector size is not 512\n");
    ...
    {
        uartOuts("bpb_is_sane() - Error: cluster size is larger than 128K\n");
    ...
    {
        uartOuts("bpb_is_sane() - Warning: NumFATS is not 2\n");
        //DEBUGF( "bpb_is_sane() - Warning: NumFATS is not 2 (%d)\n",
        //        fat_bpb.bpb_numfats);
    }
    if(fat_bpb.bpb_media != 0xf0 && fat_bpb.bpb_media < 0xf8)
    {
        uartOuts("bpb_is_sane() - Warning: Non-standard media type\n");
    ...
    {
        uartOuts("bpb_is_sane() - Error: Last word is not 0xaa55\n");
...
static void *cache_fat_sector(int fatsector)
{
    ...
    if(fce->inuse && fce->secnum != secnum)
    {
        ...
        if(fce->dirty)
        {
            rc = ataWriteSectors(fce->secnum+fat_bpb.startsector, 1,
                                   sectorbuf);
            if(rc < 0)
            {
                uartOuts("cache_fat_sector() - Could not write sector\n");
            ...
            if(fat_bpb.bpb_numfats > 1)
            {
                ...
                rc = ataWriteSectors(fce->secnum+fat_bpb.startsector+
    ...
    if(!fce->inuse)
    {
        rc = ataReadSectors(secnum + fat_bpb.startsector,1,
                              sectorbuf);
        if(rc < 0)
        {
            uartOuts("cache_fat_sector() - Could not read sector\n");
...
static int update_fat_entry(unsigned int entry, unsigned int val)
{
    ...
    uartOuts("update_fat_entry\n");
    //LDEBUGF("update_fat_entry(%x,%x)\n",entry,val);

    if (entry==val)
        uartOuts("Creating FAT loop\n");
    //    panicf("Creating FAT loop: %x,%x\n",entry,val);

    if ( entry < 2 )
        uartOuts("Updating reserved FAT entry\n");
//        panicf("Updating reserved FAT entry %d.\n",entry);

    sec = cache_fat_sector(sector);
    if (!sec)
    {
        uartOuts("update_entry() - Could not cache sector\n");
...
static int read_fat_entry(unsigned int entry)
{
    ...
    if (!sec)
    {
        uartOuts("read_fat_entry() - Could not cache sector\n");
...
static int update_fsinfo(void)
{
    ...
    /* update fsinfo */
    rc = ataReadSectors(fat_bpb.startsector + fat_bpb.bpb_fsinfo, 1,fsinfo);
    if (rc < 0)
    {
        uartOuts("flush_fat() - Couldn't read FSInfo\n");
    ...
    *intptr = SWAB32(fat_bpb.fsinfo.nextfree);
    ...
    rc = ataWriteSectors(fat_bpb.startsector + fat_bpb.bpb_fsinfo,1,fsinfo);
    if (rc < 0)
    {
        uartOuts("flush_fat() - Couldn't write FSInfo\n");
...
static int flush_fat(void)
{
    ...
    for(i = 0;i < FAT_CACHE_SIZE;i++)
    {
        if(fat_cache[i].inuse && fat_cache[i].dirty)
        {
            ...
            uartOuts("Flushing FAT sector\n");
            //LDEBUGF("Flushing FAT sector %x\n", secnum);
            sec = fat_cache_sectors[i];

            /* Write to the first FAT */
            rc = ataWriteSectors(secnum, 1, sec);
            if(rc)
            {
                uartOuts("flush_fat() - Couldn't write sector\n");
            ...
            {
                /* Write to the second FAT */
                rc = ataWriteSectors(secnum + fat_bpb.fatsize, 1, sec);
                if (rc)
                {
                    uartOuts("flush_fat() - Couldn't write sector\n");
                    //DEBUGF( "flush_fat() - Couldn't write"
...
{
    struct tm* ctime = rtcGetTime();

    if (date)
        *date = ((ctime->tm_year - 80) << 9) |
            ((ctime->tm_mon + 1) << 5) |
            ctime->tm_mday;

    if (time)
        *time = (ctime->tm_hour << 11) |
            (ctime->tm_min << 5) |
            (ctime->tm_sec >> 1);

    if (tenth)
        *tenth = (ctime->tm_sec & 1) * 100;
}
                           ...
                           unsigned char* shortname)
{
    ...
    for (i=0; i < numentries; i++) {
        ...
        if ( idx >= DIR_ENTRIES_PER_SECTOR ) {
            ...
            rc = fat_readwrite(file, 1, buf, false);
            if (rc<0) {
                uartOuts("Failed writing new sector\n");
        ...
        if (entry[0] && entry[0] != 0xe5 )
            uartOuts("Dir entry is not free!\n");
 ...
 restart:
    ...
    for (sector=0; !done; sector++)
    {
        ...
        if (rc == 0) {
            ...
            memset(buf, 0, sizeof buf);
            uartOuts("Adding new sector to dir\n");
        ...
        if (rc < 0) {
            uartOuts("add_dir_entry() - Couldn't read dir\n");
        ...
        for (i=0; i < DIR_ENTRIES_PER_SECTOR && !done; i++)
        {
            ...
            default:
                ...
                if (!strncmp(shortname, buf + i * DIR_ENTRY_SIZE, 12)) {
                    ...
                    uartOuts("Duplicate shortname DAMMIT!\n");
    ...
    uartOuts("Adding longname to entry\n");
    ...
    if (last)
    {
        ...
        uartOuts("Updating end-of-dir entry\n");
...
static int update_short_entry( struct fat_file* file, int size, int attr )
{
    ...
    if (!entry[0] || entry[0] == 0xe5)
        uartOuts("Updating size on empty dir entry\n");
               ...
               int attr)
{
    ...
    if ( !file->dircluster ) {
        uartOuts("File has no dir cluster!\n");
        ...
        if (cluster) {
            ...
            if (fat_bpb.fsinfo.freecount>0)
                panicf("There is free space, but find_free_cluster() "
                       "didn't find it!\n");
#endif
            uartOuts("next_write_cluster(): Disk full!\n");
...
static int transfer( unsigned int start, int count, char* buf, bool write )
{
    ...
    if (write) {
        if (start < fat_bpb.firstdatasector)
            uartOuts("Writing before data\n");
            //panicf("Writing before data\n");
        rc = ataWriteSectors(start, count, buf);
    }
    else
        rc = ataReadSectors(start, count, buf);
    if (rc < 0) {
        uartOuts("transfer() - Couldn't do sector\n");
...
//TODO
        ...
        for (i=0; i<numclusters; i++) {
            cluster = get_next_cluster(cluster);
            if (!cluster) {
                uartOuts("Seeking beyond the end of the file!\n");
...
int fat_opendir(struct fat_dir *dir, unsigned int startcluster)
{
    ...
    if(rc)
    {
        uartOuts("fat_opendir() - Couldn't open dir\n");
...
int fat_getnext(struct fat_dir *dir, struct fat_direntry *entry)
{
    ...
    while(!done)
    {
        if ( !(dir->entry % DIR_ENTRIES_PER_SECTOR) || !dir->sector )
        {
            ...
            if (rc < 0) {
                uartOuts("fat_getnext() - Couldn't read dir\n");
