...
int remove(const char* name)
{
    ...
    if ( rc < 0 ) {
        uartOuts("Failed truncating file\n");
...
static int flush_cache(int fd)
{
    ...
    uartOuts("Flushing dirty sector cache\n");
...
static int readwrite(int fd, void* buf, int count, bool write)
{
    ...
    if ( sectors ) {
        ...
        if ( rc < 0 ) {
            uartOuts("Failed read/writing sectors\n");
            //DEBUGF("Failed read/writing %d sectors\n",sectors);
            errno = EIO;
            if(write && file->fatfile.eof) {
                uartOuts("No space left on device\n");
    ...
    if ( count ) {
        if (write) {
            ...
            rc = fat_readwrite(&(file->fatfile), 1, &(file->cache),false);
            if (rc < 1 ) {
                uartOuts("Failed caching sector\n");
