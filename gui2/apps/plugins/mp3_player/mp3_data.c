...
static unsigned long __find_next_frame(FILE * fd, int *offset, int max_offset, unsigned long last_header, int(*getfunc)(FILE * fd, unsigned char *c))
{
...
static int fileread(FILE * fd, unsigned char *c)
{
    return fread((void*)c, 1, 1, fd);
}

unsigned long find_next_frame(FILE * fd, int *offset, int max_offset, unsigned long last_header)
{
...
static int buf_getbyte(FILE * fd, unsigned char *c)
{
    ...
    {
        fnf_buf_len = fread(mp3buf, 1, mp3end - mp3buf, fd);
        if(fnf_buf_len < 0)
            return -1;
...
static int buf_seek(FILE * fd, int len)
{
    fnf_read_index += len;
    if(fnf_read_index > fnf_buf_len)
    {
...
unsigned long buf_find_next_frame(FILE * fd, int *offset, int max_offset,
...
static int mem_getbyte(FILE * dummy, unsigned char *c)
{
    dummy = dummy;
                                  ...
                                  unsigned long last_header)
{
    ...
    return __find_next_frame(NULL, offset, max_offset, last_header, mem_getbyte);
}

int get_mp3file_info(FILE * fd, struct mp3info *info)
{
    ...
    if(fread(frame, 1, info->frame_size-4, fd) < 0)
        return -3;
...
int count_mp3_frames(FILE * fd, int startpos, int filesize,
                     void (*progressfunc)(int))
{
    ...
    if(fseek(fd, startpos, SEEK_SET) < 0)
        return -1;
...
int create_xing_header(FILE * fd, int startpos, int filesize,
