...
static int read_unsynched(FILE * fd, void *buf, int len)
{
    int i;
    int rc;
    int remaining = len;
...
static int skip_unsynched(FILE * fd, int len)
{
    int rc;
    int remaining = len;
    int rlen;
 ...
 */
static bool setid3v1title(FILE * fd, struct mp3entry *entry) 
{
    unsigned char buffer[128];
    static const char offsets[] = {3, 33, 63, 93, 125, 127};
    int i, j;

    if (-1 == fseek(fd, -128, SEEK_END))
        return false;

    if (fread(buffer, 1, sizeof buffer, fd) != sizeof buffer) 
        return false;
 ...
 * Returns: true if a title was found and created, else false
 */
static void setid3v2title(FILE * fd, struct mp3entry *entry) 
{
    int minframesize;
    int size;
    ...
    if(version >= ID3_VER_2_4) {
        if(global_flags & 0x40) {
            if(4 != fread(header, 1, 4, fd))
                return;
            
            framelen = UNSYNC(header[0], header[1], 
                              header[2], header[3]);

            fseek(fd, framelen - 4, SEEK_CUR);
        }
    }
    ...
    while(size >= minframesize ) {
        ...
        if(flags)
        {
            ...
            if (version >= ID3_VER_2_4) {
                if(flags & 0x0040) { /* Grouping identity */
                    fseek(fd, 1, SEEK_CUR); /* Skip 1 byte */
                    framelen--;
                }
            } else {
        ...
        for (i=0; i<TAGLIST_SIZE; i++) {
            ...
            if( !*ptag && !memcmp( header, tr->tag, tr->tag_length ) ) { 
                ...
                if(global_unsynch && version <= ID3_VER_2_3)
                    ...
                    bytesread = fread(buffer + bufferpos, 1, framelen, fd); 
        ...
        if( i == TAGLIST_SIZE ) {
            ...
            if(global_unsynch && version <= ID3_VER_2_3) {
                ...
                size -= totframelen;
                if( fseek(fd, totframelen, SEEK_CUR) == -1 )
                    return;
            }
        }
    }
 ...
 */
static int getid3v2len(FILE * fd) 
{
    char buf[6];
    int offset;
 ...
 */
static int getsonglength(FILE * fd, struct mp3entry *entry)
{
    ...
    if(-1 == fseek(fd, entry->id3v2len, SEEK_SET))
        return 0;
