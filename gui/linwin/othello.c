...
static void draw_oth()
{
	...
	GrSetGCForegroundPixelVal(oth_gc, AV3XX_COLOR_BLUE); // Gitter
	...
	GrSetGCForegroundPixelVal(oth_gc, AV3XX_COLOR_RED); // Auswahlrahmen
	...
	GrSetGCForegroundPixelVal(oth_gc, AV3XX_COLOR_BLACK); // Stein
...
static void oth_set_piece(int pos, int coloresq)
{
	...
	status[pos] = coloresq;
	GrSetGCForegroundPixelVal(oth_gc, AV3XX_COLOR_GREEN);//WHITE);
	if(coloresq==0)
		GrFillPoly(oth_wid, oth_gc, 5, cheese);
	else if(coloresq==1) {
   	GrSetGCForegroundPixelVal(oth_gc, AV3XX_COLOR_BLACK);
		GrFillPoly(oth_wid, oth_gc, 5, cheese);
   	GrSetGCForegroundPixelVal(oth_gc, AV3XX_COLOR_GREEN);//WHITE);
...
static int endgame(int isOver)
{
	...
	for(i=0;i<64;i++) {
  	...
  	GrSetGCForegroundPixelVal(oth_gc, AV3XX_COLOR_WHITE);//BLACK);
	GrFillRect(oth_wid, oth_gc, 32, 8, 97, 97);
  	GrSetGCForegroundPixelVal(oth_gc, AV3XX_COLOR_BLACK);//WHITE);
...
void new_oth_window()
{
	...
	oth_gc = GrNewGC();
	GrSetGCUseBackground(oth_gc, GR_TRUE);
  	GrSetGCForegroundPixelVal(oth_gc, AV3XX_COLOR_WHITE);
